...
​	本工程创建于2019/11/21，用于针对 [天池新人实战赛之[离线赛] - 阿里移动推荐算法](https://tianchi.aliyun.com/competition/entrance/231522/introduction) 赛题进行数据处理、特征工程、构建模型等步骤完成预测算法。该题属于 __广告推送 __ 型的 __多 ~~分类~~ 标签__ 赛题，评价指标为 __F1__ 值。
...
> 
>​	最后一个大问题是多 ~~分类~~ 标签怎么做，4W多的商品不可能用40W个标签或者40W个模型一个一个的得出结果，这应该是之后考虑的重点。
> 
```
2019/11/26 13:12
```
...
2019/12/15 02:35
```
>碰见麻烦事，原本的思路是取销量前几的商品做标签，然后做一个多 ~~分类~~ 标签模型。然而某些大户可能在同一天购买多种商品，这样的话单纯的多 ~~分类~~ 标签就会损失过大。莫非真的只能一个模型一件商品的去判断吗... 需要花时间思考一下。
>
>再查下 [XGBoost文档](https://xgboost.apachecn.org/#/docs/15)<sup>[5]</sup> 可知xgb可以做多 ~~分类~~ 标签，而且能调整参数  ```objective: multi:softprob``` 输出概率矩阵，这样的话我们还是可以自己定义阈值，一定程度上解决一人对多种商品的问题。
>
...
> 又是个问题，这赛题是多标签而非单单的多分类(顺便更改之前的多分类为多标签)。
>
...
> 无意间看数据玩，发现个BUG。原item列表里，同一个item可能对应多个经纬度，所以我在做总表的merge时是有问题的。现在看最好先把item的经纬度删去，再merge，之后提特征时再加回去。
>
...
>
> 第一个思路是 把前一天在购物车列表的所有商品都交上去，上下分别为全集子集商品，结果如下并不好看 。时间越久分越低，可以猜测为随着时间推移，分母商品越来越多，分子除异常不会有太大变化，所以结果越来越低。
>
> ```
> 第28天为标签
> 0.04236068270614847
...
> -----
> 
> 根据以上思路，再做一个马后炮的测试。假设预先知道正确标签，根据正确标签，用曾出现在前一天的购物车里的商品为答案。新增的有逗号的一行是按顺序统计预测和目标文件的行数。
> 
> 这分和网上别人的成绩比还可以，但是看这行数差太多，明显召回率太低了，说明有N多人不买东西不会在前一天加如购物车，总之处理神经病购买还是有难度。
> 
> ```
>第28天为标签
> 515 4367
...
> 
> 而又一个问题是什么，我们如果根据过去的行为预测未来，这在本赛题也不太理想，为什么？因为这是购物，假设我第一天买了手机，我第二天会再继续买手机吗？所以用过去预测未来，方向不能跑偏了，具体什么方向是对的也难说。比如缩小多标签就会碰到这个问题，我昨天买了手机，那我今天把手机当成要预测的标签，这肯定是不现实的。
> 
...
> -------------
> 
> 把浏览记录提升到前两天而非前一天，照样是依照标签数据做到100%准确率，结果仍然有所提升，但是数据量必然是大涨。所以这里我们面临的问题就是如何权衡准确率和召回率，需要好好思考一下，究竟是以前一天数据为准还是以前两天数据为准。天数越多，每个用户的标签也就越多，虽然召回率上升，但正负样本量必然越不平衡，保持高准确率也会越困难。
> 
> ```
> 第28天为标签
> 1297 4367
...
> 我首先要取离标签最近的一个记录，也就是取time最大的一条标签，即取 index=2 的记录。所以我对time和user_id进行rank：
>
> | index | user_id | item_id | ui_id    | time |
> | :---- | :------ | :------ | :------- | :--- |
> | 2     | xiao    | hua     | xiaohua  | 16   |
...
> 今天（啊，第二天的凌晨都算在今天内~~）记录的有点晚，因为一直在跑模型，但是效果还是有的，现在的分已经在赛题的前15名了。
>
...
>
> 另外一个是按照本赛题自定义了XGBOOST的评测函数(非损失函数)，观察发现可能迭代次数过多了，因为验证集无论怎么弄，总会在300次迭代内得到最好的验证集评测分数，所以今晚交了减少迭代次数交了答案，分数上涨。
>
...
<sup>特别提示：如果  "特征" 后标有  ```含BUG```，说明该记录中的特征提取有问题；具体问题详见对应 [特征方案](特征方案)，根据提交时间和方案内的BUG修改时间进行核对。</sup>

```
2019/12/28 18:29 _Z/beh_type_3&latest_1_day_201912281826.csv 0.05877
```

> 提交 测试集前一天，所有被加入购物车的子集商品和对应的用户记录，共1831条。

```
2019/12/29 20:50 _A_01_ui_xgb_201912292046.csv  0.00000 ？？？
```

> 1. 特征： _A_01 特征第三部分 ```含BUG```
> 2. 模型：XGBOOST
> 3. 模型参数：XGB_01 除去 ``` tree_method': exact ```
> 4. 训练 duration=31
> 5. 按概率取前 900 提交
> 6. 取前标签前 一 天所有 浏览 记录为标签
> 7. 31取验证集标签，31取训练集标签

```
2019/12/29 23:52 _A_01_xgb_201912292341.csv 0.06347
```
...
```
2019/12/30 23:51 _A_01_ui_xgb_201912302333.csv 0.04906
```

> 1. 特征： _A_01 第三部分 ```含BUG```
>2. 模型：XGBOOST
> 3. 模型参数：XGB_01
>4. 训练 duration=31
> 5. 按概率取前700提交
>6. 取前标签前 七 天所有 浏览 记录为标签
> 7. 31取验证集标签，31取训练集标签

```
2019/12/31 20:51 _A_02_ui_xgb_201912312051.csv 0.08039
```

> 1. 特征： _A_02 第三部分 ```含BUG```
>2. 模型：XGBOOST
> 3. 模型参数：XGB_01
>4. 训练 duration=31
> 5. 按概率取前 900 提交
...
```
2019/12/31 21:30 _A_02_xgb_201912312051.csv 0.08604
```
...
```
2020/01/01 11:30 _A_02_tl_xgb_202001011106.csv 0.06629
```

> 1. 迁移学习
> 2. 特征： _A_02 ，第一部分 和 第二部分 按同模型分别预测用户和商品购买概率加入第三部分 ```含BUG```
> 3. 模型：XGBOOST
> 4. 模型参数：XGB_01
> 5. 训练 duration=31
> 6. 按概率取前 900 提交
> 7. 取前标签前 一 天所有 浏览 记录为标签
> 8. 31取验证集标签，31取训练集标签
>

```
2020/01/01 21:26 _A_02_xgb_202001012126.csv 0.08035
```
...
```
2020/01/01 21:26 _A_02_xgb_202001012126.csv 0.07475
```

> 1. 特征： _A_02 除去所有与子集相关的特征  ```含BUG```
> 2. 模型：XGBOOST
> 3. 模型参数：XGB_01
> 4. 训练 duration=31
> 5. 按概率取前 900 提交
...
```
2020/01/02 11:11 _A_02_xgb_202001021111.csv 0.09031
```
...
```
2020/01/02 11:38 _A_02_xgb_202001021138.csv 0.09430
```
...
```
2020/01/02 19:13 _A_02_xgb_202001021913.csv 0.08644
```
...
```
2020/01/02 19:57 _A_02_xgb_202001021957.csv 0.08644 
```
...
>4. 训练 duration=31
> 5. 按概率取前 500 提交
>6. 取前标签前 三 天所有 购物车 记录为标签
>    

```
2020/01/02 22:16 _A_02_xgb_202001022205.csv 0.09037 
```
...
> 4. 训练 duration=31
> 5. 按概率取前 500 提交
> 6. 取前标签前 三 天所有 浏览 记录为标签
> 7. 31取验证集标签，31取训练集标签
>

```
2020/01/03 11:53 _A_02_xgb_202001022205.csv 0.08497 
```
...
```
2020/01/03 19:09 _A_02_xgb_202001031905.csv 0.09037 
```
...
```
2020/01/03 19:59 _A_02_xgb_202001031927.csv 0.08251 
```
...
```
2020/01/03 20:19 _A_02_xgb_202001032006.csv 0.07859 
```
...
> 5. 按概率取前 500 提交
> 6. 取前标签前 一 天所有 浏览 记录为标签
> 7. 32取验证集标签，30 31 取训练集标签

```
2020/01/03 23:43 _A_02_xgb_202001032331.csv 0.09823 
```
...
```
2020/01/05 00:14 _A_02_xgb_202001050001.csv 0.09234 
```
...
> 6. 取前标签前 三 天所有 浏览 记录为标签
> 7. 32取验证集标签，31取训练集标签
> 8. 剔除了只在 day_rank=25(双12) 出现的数据

```
2020/01/05 00:43 _A_02_xgb_202001050031.csv 0.10216
```
...
> 4. 训练 duration=31
> 5. 按概率取前 500 提交
> 6. 取前标签前 三 天所有 交互 记录为标签
> 7. 32取验证集标签，31取训练集标签

```
2020/01/05 00:43 _A_02_xgb_202001050031.csv 0.10413
```

> 1. 特征： _A_02
> 2. 模型：XGBOOST
> 3. 模型参数：XGB_01  ```num_round=300```
> 4. 训练 duration=31
> 5. 按概率取前 500 提交
> 6. 取前标签前 三 天所有 交互 记录为标签
> 7. 32取验证集标签，31取训练集标签
